...
# 인자 없으면 전체, 있으면 해당 데모만 (예: bash scripts/verify_all.sh demo1 demo4)
ONLY="${*:-all}"
...
want demo2 && run demo2 env LLM_PROVIDER=mock DEMO2_AUTO=all $NB demo2_hitl/demo2.ipynb

# ── 데모 4: OTel 계측 (오프라인 console exporter) ──
want demo4 && run demo4 env LLM_PROVIDER=mock OTEL_TARGETS=console $NB demo4_otel_swap/demo4.ipynb

# ── 데모 5: deepagents (내부 assert: 브리프 파일 산출) ──
want demo5 && run demo5 env LLM_PROVIDER=mock $NB demo5_deepagents/demo5.ipynb
